...
│   └── g1-dialogue.service
└── docs/
    ├── ubtech-migration-guide.md
    ├── 2026-09-03-ubtech-migration-worklog.md
...
- `dialogue_core.py`：可供不同机器人复用的知识库、Qwen和上下文逻辑。
- `ubtech_adapter.py`：对接优必选 ROS2 鉴权、状态、文本播报、动作和播放完成事件。
- `ubtech_dialogue.py`：优必选状态检查、直接播报、动作测试和智慧问答入口。
- `run_ubtech_mock_smoke.py`：无实体机器人时验证迁移控制流的十轮模拟测试。
...
优必选迁移代码已经加入本仓库，详细上机流程见：

```text
docs/ubtech-migration-guide.md
```
...
进入优必选官方 `demo_runtime` 容器并加载 ROS2 环境后，按以下顺序验证：
...
`play_text` 返回成功只表示请求被受理。本实现必须收到 `/robo/media/subscribe/playback_state` 的 `phase=result` 事件才把本轮记为完成，避免将“接口成功、机器人未播报”误判为闭环。
